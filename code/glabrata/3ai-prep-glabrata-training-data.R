## Set up (thin points) training-validation data for B. glabrata model
...
occ.target <- environmental_covariate_df %>% 
  filter(str_detect(species, "glabrata")) #%>% #filter by target species
...
thin.occ <- occ.target[row.names(s),]; thin.occ <- thin.occ[complete.cases(thin.occ), ] #165 total
thin.occ$presence <- 1
...
bg_df <- environmental_covariate_df %>%
  filter(str_detect(species, "glabrata", negate = TRUE))
...
#combine data
names(bg_final_data)
all_data <- rbind(thin.occ, bg_final_data[-36]) #"index" #495 total
...
saveRDS(analysis_data_final_AR, "clean-data/glabrata/glabrata_AR_brt_data_oct12.rds")
saveRDS(analysis_data_final_UN, "clean-data/glabrata/glabrata_UN_brt_data_oct12.rds")
