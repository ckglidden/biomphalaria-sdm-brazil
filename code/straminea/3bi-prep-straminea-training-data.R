## code to clean training and validation data for B. straminea model (2000-2017 data)
...
##--------------------------------##
# set target species
target_species_string <- 'straminea' #, 'tenagophila', 'straminea'

occ.target <- environmental_covariate_df %>% 
  filter(str_detect(species, "straminea")) #%>% #filter by target species
...
thin.occ <- occ.target[row.names(s),]; thin.occ <- thin.occ[complete.cases(thin.occ), ] #283 total
thin.occ$presence <- 1
...
bg_df <- environmental_covariate_df %>%
  filter(str_detect(species, "straminea", negate = TRUE))
...
#combine data
names(bg_final_data)
all_data <- rbind(thin.occ, bg_final_data[-36]) #"index" #849 total
...
saveRDS(analysis_data_final_AR, "clean-data/straminea/straminea_AR_brt_data_oct12.rds")
saveRDS(analysis_data_final_UN, "clean-data/straminea/straminea_UN_brt_data_oct12.rds")
